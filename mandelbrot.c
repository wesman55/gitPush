/*
  ...
  1a) the number of (virtual) cores on your machine: 20

  1b) your compute times (not file writing) for 1, 2, 4, and 8 threads:


 	1 thread: Time: 30.040411 seconds

	2 threads: Time: 16.012419 seconds

	4 threads: Time: 9.398917 seconds

	8 threads: Time: 5.153736 seconds	
*/
...
int main(int argc, char* argv[])
{
  ...
  char *buffer = calloc(xres * yres * 6, sizeof(char));
  
  omp_set_num_threads(8);
  double startTime = omp_get_wtime();
  #pragma omp parallel for private(x, y, u, v, i, k) schedule(static)
  for (j = 0; j < yres; j++) {
    y = ymax - j * dy;
    for(i = 0; i < xres; i++) {
      double u = 0.0;
	...
	memcpy(buffer + (j * (xres * 6) + i * 6), black, 6);
        //fwrite (black, 6, 1, fp);
      }
      else {
        /* exterior */
        ...
        color[4] = k >> 8;
        color[5] = k & 255;
	memcpy(buffer + (j * (xres * 6) + i * 6), color, 6);
        //fwrite(color, 6, 1, fp);
      };
    }
  }
  ...
  fwrite(buffer, xres * yres * 6, 1, fp);
  fclose(fp);
  return 0;
}
